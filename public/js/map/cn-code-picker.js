/** Accessible dropdown menu listing every CN code, layered over the datalist input. */

export function openFullCodeMenu(codes, currentValue) {
...
export function moveActiveIndex(currentIndex, direction, length) {
  if (length <= 0) return -1;
  const start = currentIndex < 0 ? (direction < 0 ? 0 : -1) : currentIndex;
  return (start + direction + length) % length;
}

export function commitSelectedCode(input, code) {
  input.value = code;
  input.dispatchEvent(new Event("change", { bubbles: true }));
  input.blur();
}

export function initCnCodePicker() {
  ...
  if (!input || !datalist || !toggle || !menu || !shell) return;
